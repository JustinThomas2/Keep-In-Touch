    ...
    <section class="page-shell">
      <ng-container *ngIf="company$ | async as company; else loading">
        <div class="page-header" *ngIf="company; else missing">
          ...
          <a class="button is-link" [routerLink]="['/contacts/new']" [queryParams]="{ companyId: company.id }">
            New contact
          </a>
        </div>
        ...
        <div class="detail-panel" *ngIf="company">
          <div class="detail-grid">
            <div class="detail-item">
              ...
              <a class="detail-value" *ngIf="company.website; else empty" [href]="company.website" target="_blank">
                {{ company.website }}
              </a>
            </div>
            <div class="detail-item">
              <span class="detail-label">Location</span>
              <div class="detail-value">{{ company.location || 'Not set' }}</div>
            </div>
            <div class="detail-item is-full">
              <span class="detail-label">Notes</span>
              <div class="detail-value">{{ company.notes || 'No notes yet' }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="empty-state" *ngIf="company && company.contacts.length === 0">
          <p class="has-text-weight-semibold">No contacts associated yet.</p>
        </div>

        <div class="list-grid" *ngIf="company && company.contacts.length > 0">
          <a class="list-row" *ngFor="let contact of company.contacts" [routerLink]="['/contacts', contact.id]">
            <div>
              <div class="row-title">{{ contact.firstName }} {{ contact.lastName || '' }}</div>
              <div class="row-meta">{{ contact.roleTitle || contact.email || 'No role or email' }}</div>
            </div>
