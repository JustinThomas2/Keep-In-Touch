...
class DomainPersistenceTests {
	...
	void savesAndQueriesCoreDomainGraph() {
		User user = userRepository.save(new User("phase4-" + UUID.randomUUID() + "@keep-in-touch.test", "Phase 4"));
		...
		FollowUp savedFollowUp = followUpRepository.findByContactIdAndStatus(contact.getId(), FollowUpStatus.OPEN)
				.orElseThrow();
