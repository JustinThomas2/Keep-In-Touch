...
public class FollowUpService {
	...
	public List<FollowUp> findOverdueByUserId(UUID userId, OffsetDateTime now) {
		return followUpRepository.findByContactUserIdAndStatusAndDueAtLessThanOrderByDueAtAsc(userId, FollowUpStatus.OPEN,
				now);
	}

	public List<FollowUp> findDueByUserId(UUID userId, OffsetDateTime now) {
		OffsetDateTime tomorrow = now.plusDays(1);
		return followUpRepository.findByContactUserIdAndStatusAndDueAtBetweenOrderByDueAtAsc(userId, FollowUpStatus.OPEN,
				now, tomorrow);
	}
	...
	@Transactional
	public FollowUp create(UUID userId, UUID contactId, UUID interactionId, OffsetDateTime dueAt, String reason) {
		Contact contact = contactRepository.findByIdAndUserId(contactId, userId)
				.orElseThrow(() -> new IllegalArgumentException("Contact not found."));
		findOpenByContactId(contactId).ifPresent(existing -> {
		...
		if (interactionId != null) {
			Interaction interaction = interactionRepository.findByIdAndContactUserId(interactionId, userId)
					.filter(candidate -> candidate.getContact().getId().equals(contactId))
					.orElseThrow(() -> new IllegalArgumentException("Interaction not found for contact."));
	...
	public FollowUp complete(UUID userId, UUID followUpId) {
		FollowUp followUp = findByIdAndUserId(followUpId, userId)
	...
	private void updateNextFollowUpAt(Contact contact) {
		OffsetDateTime nextFollowUpAt = followUpRepository
