...
public class InteractionService {
	...
	public Interaction create(
		...
		Interaction interaction = new Interaction(contact, requireNonNull(interactionType, "Interaction type is required."),
				requireNonNull(occurredAt, "Occurred at is required."), requireText(summary, "Summary is required."));
	...
	public Interaction update(
			...
			String summary,
			String outcome) {
		Interaction interaction = findByIdAndUserId(interactionId, userId)
				.orElseThrow(() -> new IllegalArgumentException("Interaction not found."));
		interaction.setInteractionType(requireNonNull(interactionType, "Interaction type is required."));
	...
	private void updateLastInteractionAt(Contact contact) {
		OffsetDateTime lastInteractionAt = interactionRepository.findTopByContactIdOrderByOccurredAtDesc(contact.getId())
